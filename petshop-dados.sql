...
--
-- Host: localhost:3307
-- Generation Time: Oct 04, 2015 at 10:08 PM
...
(3, 'Félix', '2015-04-13', 'GATO', NULL, 'CINZA', 2);
...
ALTER TABLE `agenda_animal`
  ADD PRIMARY KEY (`agenda_codigo`,`animal_codigo`,`procedimento_id`),
  ADD KEY `animal_fk` (`animal_codigo`),
  ADD KEY `procedimento_fk` (`procedimento_id`);
...
ALTER TABLE `animal`
  ADD PRIMARY KEY (`codigo`),
  ADD UNIQUE KEY `nome` (`nome`,`dono_codigo`),
  ADD KEY `dono_fk` (`dono_codigo`);
...
--
ALTER TABLE `animal`
  ADD CONSTRAINT `dono_fk` FOREIGN KEY (`dono_codigo`) REFERENCES `dono` (`codigo`);
